...
export const monitorsRouter = {
        ...
        .handler(async ({ input, context }) => {
            ...
            const loadMonitorState = async () => {
                ...
                try {
                    const monitorStatusInputs = monitors.map((row) => ({
                        id: row.monitor.id,
                        workerIds:
                            (row.monitor.workerIds as string[] | null) ?? [],
                        locations:
            ...
            const monitorsWithStatus = monitors.map((row) => {
                ...
                return {
                    ...
                    notificationCount:
                        notificationCountMap.get(row.monitor.id) || 0,
                    activeIncidentId:
                        activeIncidentMap.get(row.monitor.id) ?? null,
                };
            });
            ...
            const newMonitor = await db.transaction(async (tx) =>
                insertMonitor(tx, {
                    ...
                    publishIncidentToStatusPage:
                        input.publishIncidentToStatusPage,
                    tagIds: input.tags ?? [],
                    notificationIds,
                }),
            );
    ...
    delete: writeProcedure
        ...
        .handler(async ({ input, context }) => {
            ...
            if (activeIncidentLinks.length > 0) {
                const incidentIds = activeIncidentLinks.map(
        ...
        .handler(async ({ input, context }) => {
            ...
            await db.transaction(async (tx) => {
                await tx
                    ...
                    .set({
                        ...
                        incidentRecoveryDuration:
                            input.incidentRecoveryDuration,
                        publishIncidentToStatusPage:
                            input.publishIncidentToStatusPage,
                        active: input.active,
                        pauseReason: null,
                ...
                if (input.tags) {
                    ...
                    await tx
                        .delete(monitorTag)
                        .where(eq(monitorTag.monitorId, input.id));
    ...
    bulkAssignWorkers: writeProcedure
        ...
        .handler(async ({ input, context }) => {
            ...
            if (monitors.length !== monitorIds.length) {
                throw new ORPCError("NOT_FOUND", {
                    message:
                        "One or more selected monitors could not be found.",
                });
            }
            ...
            if (quotaState.regionsPerMonitorLimit !== null) {
                ...
                const overflowCount = monitors.filter(
                    (monitorRecord) =>
                        (nextWorkerIdsByMonitor.get(monitorRecord.id)?.length ??
                            0) > regionLimit,
                ).length;
                ...
                for (const monitorRecord of monitors) {
                    ...
                    const nextLocations = [
                        ...new Set(
                            nextWorkerIds
                                .map((workerId) =>
                                    locationByWorkerId.get(workerId),
                                )
                                .filter(
    ...
    getStatusCodeDistribution: protectedProcedure
        .route({
            ...
            description:
                "Retrieve historical HTTP status code counts for a monitor.",
        })
        .input(
            z.object({
                monitorId: z.string(),
        ...
        .handler(async ({ input, context }) => {
            ...
            const calculateStats = (startDate: Date | null) => {
                ...
                return {
                    ...
                    maxIncidentMs,
                    avgIncidentMs:
                        incidentCount > 0 ? incidentSumMs / incidentCount : 0,
                };
            };
    ...
    getBatchLatencySparkline: protectedProcedure
        ...
        .handler(async ({ input, context }) => {
            ...
            const filteredIds = monitorIds.filter((id) =>
                    ...
                    and(
                        ...
                        inArray(
                            ...
                            relatedIncidents.map(
                                (relatedIncident) => relatedIncident.id,
                            ),
                        ),
                    ),
                );
