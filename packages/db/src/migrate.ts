...
const waitForPostgres = async (
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            ...
            if (attempt > 1) {
                console.log(
                    `✅ PostgreSQL became ready after ${attempt} attempts`,
                );
            }
            return;
        } catch (error) {
            ...
            console.log(
                `⏳ Waiting for PostgreSQL... (${attempt}/${maxAttempts})`,
            );
            await sleep(delayMs);
        }
    }
...
const waitForClickHouse = async (
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            ...
            if (attempt > 1) {
                console.log(
                    `✅ ClickHouse became ready after ${attempt} attempts`,
                );
            }
            return;
        } catch (error) {
            ...
            console.log(
                `⏳ Waiting for ClickHouse... (${attempt}/${maxAttempts})`,
            );
            await sleep(delayMs);
        }
    }
...
const syncMigrationJournal = async (
    ...
    console.log(
        `✅ Marked ${missingMigrations.length} migration(s) as applied`,
    );
};
